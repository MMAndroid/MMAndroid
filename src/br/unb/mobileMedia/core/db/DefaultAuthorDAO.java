...
public class DefaultAuthorDAO implements AuthorDAO {
	...
	public void saveAuthor(Author author) throws DBException {
		try {
			...
			Cursor cursor = db.rawQuery(DBConstants.SELECT_AUTHORS_BY_NAME, new String[] { author.getName() });
	...
	public Author findByName(String name) {
		db = dbHelper.getReadableDatabase();
		Cursor cursor = db.rawQuery(DBConstants.SELECT_AUTHORS_BY_NAME, new String[] { name } );
	 ...
	 */
	public void saveAuthorProduction(Author author, List<Audio> listOfAudio) throws DBException {
		try { 
			...
			for(Audio audio: listOfAudio) {	
				Cursor cursor = db.rawQuery(DBConstants.SELECT_AUDIO_BY_AUTHOR_KEY_TITLE, new String[] { dbAuthor.getKey().toString(), audio.getTitle()});
				
				//here we check whether the audio exists or not. if it already exists,
				//we do not import it again. we are assuming that no author will produce 
				...
				if(cursor.getCount() == 0) {
					...
					values.put(DBConstants.AUDIO_URL_COLUMN, audio.getURI().toASCIIString());
					values.put(DBConstants.AUDIO_FK_AUTHOR_COLUMN, author.getId());
	...
	public List<Audio> findAudioProductionByAuthorKey(Integer key) throws DBException {
		try { 
			db = dbHelper.getReadableDatabase();
			Cursor cursor = db.rawQuery(DBConstants.SELECT_AUDIO_PRODUCTION_BY_AUTHOR_KEY, new String [] { key.toString() });
	...
	public List<Audio> listAllProduction() throws DBException {
		try {
			...
			Cursor cursor = db.rawQuery(DBConstants.LIST_AUDIO_PRODUCTION, null);
			List<Audio> production = iterateOverAudioCursor(cursor);
			cursor.close();
			return production;
		}
	 ...
	 */
	public Map<Audio, List<Date>> executionHistory(Date start, Date end) throws DBException {
		db = dbHelper.getReadableDatabase();
		...
		Map<Audio, List<Date>> result = new HashMap<Audio, List<Date>>();
		...
		if(cursor.getCount() > 0 && cursor.moveToFirst()){
			do {
				...
				try {
					dateExecution = formater.parse(cursor.getString(cursor.getColumnIndex(DBConstants.EH_DATE_TIME_EXECUTION_COLUMN)));
				}
				catch(ParseException e) {
					throw new RuntimeException(e);
				}
				
				List<Date> dates = result.get(audio);
				
				if(dates == null) {
					dates =  new ArrayList<Date>();
				}
				dates.add(dateExecution);
				
				result.put(audio, dates);
			} while(cursor.moveToNext());
		}
		cursor.close();
		return result;
	...
	public void saveExecutionHistory(Audio audio, Date time) throws DBException {
		try {
			...
			values.put(DBConstants.EH_FK_AUDIO_COLUMN, DateFormat.format(MMConstants.DATA_FORMAT, time).toString());
	...
	private Audio cursorToAudio(Cursor cursor)  {
		Integer pk = cursor.getInt(cursor.getColumnIndex(DBConstants.AUDIO_KEY_COLUMN));
		Integer id = cursor.getInt(cursor.getColumnIndex(DBConstants.AUDIO_ID_COLUMN));
		String title = cursor.getString(cursor.getColumnIndex(DBConstants.AUDIO_TITLE_COLUMN));
		String album = cursor.getString(cursor.getColumnIndex(DBConstants.AUDIO_ALBUM_COLUMN));
	...
	private Author cursorToAuthor(Cursor cursor) {
		Integer key = cursor.getInt(cursor.getColumnIndex(DBConstants.AUTHOR_KEY_COLUMN));
